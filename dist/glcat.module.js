...
class GLCatTexture {
     ...
     */
    setTextureFromArray(width, height, source, format = GL_RGBA) {
        const { gl } = this.__glCat;
        this.__glCat.bindTexture2D(this, () => {
     ...
     */
    setTextureFromFloatArray(width, height, source, format = GL_RGBA) {
        const { gl } = this.__glCat;
...
class GLCat {
     ...
     */
    lazyMultisampleFramebuffer(width, height, { samples = 4, isFloat = false, depthFormat = this.preferredDepthFormat, fallback = true } = {}) {
        const gl = this.__gl;
        if (gl instanceof WebGL2RenderingContext) {
